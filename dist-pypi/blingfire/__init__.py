...
def text_to_words_with_tags_with_model(h, s):
    ...
    o_tags = (c_int32 * o_bytes_count)()

    # identify words
    o_len = blingfire.TextToWordsWithTagsWithModel(c_char_p(s_bytes), c_int(len(s_bytes)), byref(o_bytes), byref(o_tags), c_int(o_bytes_count), c_void_p(h))

    num_tokens = o_bytes.value.count(ord(' ')) + 1
    ...
    # compute the unicode string from the UTF-8 bytes
    return (o_bytes.value.decode('utf-8').split(' '), o_tags[:num_tokens])
